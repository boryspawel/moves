...
export class SessionsPage {
  ...
  async start(): Promise<void> { const session = this.today(); const revision = this.agenda()?.activePlan?.activeRevisionId; if (!session?.sessionId || !revision) return; await this.api.safety.checkIn({ checkInRequest: this.checkIn.getRawValue() }); const attempt = await this.api.attempts.start({ idempotencyKey: crypto.randomUUID(), startAttemptCommand: { plannedSessionId: session.sessionId, planRevisionId: revision, selectedVariantType: 'STANDARD' } }); if (attempt.attemptId) await this.open(attempt.attemptId, true); }
