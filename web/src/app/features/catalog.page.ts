...
export class CatalogPage {
  ...
  constructor() { inject(Title).setTitle('Katalog ćwiczeń | Moves'); this.route.queryParamMap.pipe(switchMap(params => { const next = this.read(params); this.state.set(next); this.invalid.set(next.invalid); this.query.setValue(next.query, {emitEvent:false}); this.pattern.setValue(next.movementPattern, {emitEvent:false}); this.level.setValue(next.technicalLevel, {emitEvent:false}); this.equipment.setValue(next.equipment, {emitEvent:false}); this.loading.set(true); this.error.set(false); return this.api.list1({query:next.query || undefined, movementPattern:next.movementPattern as any || undefined, technicalLevel:next.technicalLevel as any || undefined, equipment:next.equipment || undefined, page:next.page, size:next.size}); }), takeUntilDestroyed(this.destroyRef)).subscribe({ next: page => { this.items.set(page.content ?? []); this.total.set(page.totalElements ?? 0); this.loading.set(false); }, error: () => { this.error.set(true); this.loading.set(false); }}); }
  label = catalogLabel;
