...
export interface AppointmentLayout { appointment: AppointmentView; top: number; height: number; column: number; columns: number; }
...
export function defaultVisibleRange(appointments: AppointmentView[], availability: AvailabilityWindowView[], fallbackDate: string): VisibleRange {
  const times = [...appointments, ...availability].flatMap(item => [new Date(item.startsAt ?? Number.NaN), new Date(item.endsAt ?? Number.NaN)]).filter(value => !Number.isNaN(value.getTime()));
...
export function layoutOverlappingAppointments(appointments: AppointmentView[], range: VisibleRange): AppointmentLayout[] {
  const visible = appointments.filter((item): item is AppointmentView & { appointmentId: string; startsAt: Date; endsAt: Date } => item.status !== 'CANCELLED' && typeof item.appointmentId === 'string' && item.startsAt instanceof Date && item.endsAt instanceof Date && clampToVisibleRange(item.startsAt, item.endsAt, range) !== null);
  const sorted = [...visible].sort((a, b) => a.startsAt.getTime() - b.startsAt.getTime() || a.endsAt.getTime() - b.endsAt.getTime() || a.appointmentId.localeCompare(b.appointmentId));
  const groups: Array<typeof visible> = []; let group: typeof visible = []; let groupEnd = -Infinity;
  for (const item of sorted) { const start = item.startsAt.getTime(); if (group.length && start >= groupEnd) { groups.push(group); group = []; groupEnd = -Infinity; } group.push(item); groupEnd = Math.max(groupEnd, item.endsAt.getTime()); }
  if (group.length) groups.push(group);
  return groups.flatMap(items => {
    const columns: number[] = [];
    const assigned = items.map(appointment => { const start = appointment.startsAt.getTime(); const end = appointment.endsAt.getTime(); let column = columns.findIndex(lastEnd => lastEnd <= start); if (column < 0) { column = columns.length; columns.push(end); } else columns[column] = end; return { appointment, column }; });
