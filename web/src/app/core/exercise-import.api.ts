...
export interface CreateImportSource {
  code: string;
  displayName: string;
...
export interface EditorialDetail {
  editor: {
    ...
    loadCharacteristics: Array<Record<string, unknown>>;
...
export class ExerciseImportApi {
  ...
  sources(): Promise<ImportSource[]> {
    return this.get('/exercise-import/sources');
  }
  createSource(source: CreateImportSource): Promise<ImportSource> {
    return this.json('/exercise-import/sources', source);
  }
  batch(id: string): Promise<ImportBatch> {
    return this.get(`/exercise-import/batches/${id}`);
  }
  ...
  record(id: string): Promise<RecordDetail> {
    return this.get(`/exercise-import/records/${id}`);
  }

  issues(id: string): Promise<ImportIssue[]> {
    return this.get<string>(`/exercise-import/batches/${id}/issues?format=jsonl`).then((text) =>
      text
        .trim()
        .split('\n')
        .filter(Boolean)
        .map((line) => JSON.parse(line) as ImportIssue),
    );
  }
  ...
  async downloadIssues(id: string, format: 'csv' | 'jsonl'): Promise<void> {
    ...
    const response = await fetch(
      `${this.root}/exercise-import/batches/${id}/issues?format=${format}`,
  ...
  async upload(
    ...
    return this.request(
      `/exercise-import/batches?sourceId=${encodeURIComponent(sourceId)}&forceReprocess=${forceReprocess}`,
      {
        method: 'POST',
        body: form,
  ...
  review(
    ...
    return this.json(`/exercise-versions/${versionId}/reviews`, {
      area,
      decision,
      comment,
      expectedVersion,
  ...
  reviewStatus(versionId: string): Promise<ReviewResult> {
    return this.get(`/exercise-versions/${versionId}/reviews`);
  }
  ...
  editorialDetail(versionId: string): Promise<EditorialDetail> {
    return this.get(`/exercise-review/items/${versionId}`);
  }
  updateEditorialDraft(
    ...
    command: {
      ...
      version: Omit<
        EditorialVersion,
        'exerciseId' | 'canonicalName' | 'exerciseVersionId' | 'versionNumber' | 'status'
      >;
      expectedVersion: number;
    },
  ): Promise<unknown> {
    return this.request(`/exercises/versions/${versionId}/editorial`, {
      method: 'PUT',
      body: JSON.stringify(command),
      headers: { 'Content-Type': 'application/json' },
    });
  }
  diff(versionId: string): Promise<unknown> {
    return this.get(`/exercise-versions/${versionId}/diff`);
  }
  ...
  private async request<T>(path: string, init: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new ExerciseImportApiError(
        response.status,
        response.status === 409 ? `Konflikt (HTTP 409): ${detail}` : detail,
      );
    }
