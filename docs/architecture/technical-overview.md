...
- Akcje lifecycle terminu są wersjonowane i idempotentne; wynikają z jawnej akcji specjalisty, nigdy z odczytu `Today` ani zadania operacyjnego. Publiczne zdarzenie terminu ma własny UUID, a endpoint szczegółu jest autoryzowany w kontekście uczestnika: wymaga aktywnej relacji i capability właściwych dla tego kontekstu. Ten UUID służy jako deep link do szczegółu, bez rozszerzania dostępu poza ten kontekst.
